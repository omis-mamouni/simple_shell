...
/**
 * character_is_delimiter - check if a character is delimiter
...
/**
 * Count_number_of_token - counts the number of token in a string
 * @str: the giving string
...
int Count_number_of_token(char *str, const char *delim)
{
	...
	for (i = 0; str[i]; i++)
	{
		if (character_is_delimiter(str[i], delim))
			isNewWord = 1;
		else if (isNewWord == 1)
		{
			count++;
...
/**
 * _strtoken - splits a string into tokens
 * @str: the giving string
 * @delim: a string of delimiters
...
char **_strtoken(char *str, const char *delim)
{
	...
	if (str == NULL)
		return (NULL);
	nbrTokens = Count_number_of_token(str, delim);
	if (nbrTokens == 0)
		return (NULL);
	...
	for (i = 0; i <= len; i++)
	{
		if (str[i] && !character_is_delimiter(str[i], delim) && tLen++ == 0)
			tStart = i;
		else if (character_is_delimiter(str[i], delim) || !str[i])
		{
			if (tLen)
			{
				tEnd = i - 1;
...
/**
 * _strDuplicated - returns a pointer to duplicated string
 * @str: the giving string
