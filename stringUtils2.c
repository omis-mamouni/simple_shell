...
/**
 * _strCharacter - locate character in a string
...
/**
 * _memocpy - copy n characters from src to dest
...
/**
 * _reallocMemo - reallocates a memory block using malloc and free
...
void *_reallocMemo(void *ptr, size_t old_size, size_t new_size)
{
	...
	if (new_size > old_size)
		_memocpy(newptr, ptr, old_size);
	else
		_memocpy(newptr, ptr, new_size);
	free(ptr);
	return (newptr);
}
...
/**
 * reallocate_memory - reallocate memory
...
int reallocate_memory(char **lineptr, ssize_t n)
{
	...
	newLineptr = _reallocMemo(*lineptr, _strlen(*lineptr), n);
	if (!newLineptr)
	{
		free(*lineptr);
		return (0);
